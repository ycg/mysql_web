...
# 支持binlog查看，闪回等功能
# 安装第三方模块：pip install mysql-replication pymysql
# 调用示例：
...
# --start-datetime
# --end-datetime
# --out-file：分析的SQL保存的文件路径
# --databases：需要过滤的数据库名，通过逗号分割
# --tables：需要过滤的表名，需要跟databases参数一起使用，可以指定多个，通过逗号分割
# -B：生成回滚的SQL，也就是闪回的功能

# 注意：如果不知道binlog文件，那么就会读取数据库保存的所有binlog文件，要切记
