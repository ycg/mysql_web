...
def get_general_logs_by_page_index(page_number):
    sql = """select t1.checksum, left(t1.fingerprint, 100) as fingerprint, t1.first_seen, last_seen,
             (
...
def get_general_log_detail(checksum):
    sql = """select t1.checksum, t1.fingerprint, t1.first_seen, last_seen, t2.sample, sum(t2.ts_cnt) as ts_cnt
